...
const reducer = (state = initialState, action) => {
   switch (action.type) {
      ...
      case SEARCH_COUNTRIES:
         ...
         search = search.filter((e) => e.name.toLowerCase().includes(action.payload.toLowerCase()));
      ...
      case FILTER_BY_CONTINENT:
         ...
         const statusFilter =
            action.payload === "All" ? countries : countries.filter((el) => el.continent === action.payload);
      ...
      case ORDER_ACTIVITIES:
         ...
         createdFilter = countriesAct.filter((el) => el.Activities.length > 0);
         if (action.payload.length === 0) {
            return {
               ...state,
            };
         ...
         for (let i = 0; i < createdFilter.length; i++) {
            for (let j = 0; j < createdFilter[i].Activities.length; j++) {
               if (createdFilter[i].Activities[j].name === action.payload) {
